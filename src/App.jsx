  ...
  return (
    <div className="min-h-screen bg-[#F7F5F0] text-[#1C2B39]" style={{ fontFamily: "'Source Sans 3', 'Inter', sans-serif" }}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        {error && (
          <div className="bg-[#fcebeb] border border-[#f3d3d3] p-4 rounded-sm text-sm text-[#C1392B] flex flex-col gap-2">
            <p className="font-semibold flex items-center gap-2"><AlertTriangle size={16} /> Analysis failed</p>
            <p>{error}</p>
            <p className="mt-2 text-[#1C2B39]">
              API analysis is unavailable. You can fall back to the deterministic guidance above, or refer to the official NICE NG136 PDF document.
            </p>
            <a 
              href="/hypertension-guidance.pdf" 
              target="_blank" 
              rel="noreferrer"
              className="inline-flex items-center gap-2 bg-[#2D6E6E] text-white px-3 py-2 rounded-sm font-medium hover:bg-[#205252] transition-colors w-fit mt-1"
            >
              <BookOpen size={16} /> Open NICE NG136 PDF Fallback
            </a>
          </div>
        )}
